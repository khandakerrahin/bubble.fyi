...
public class UserDBOperations {
	 ...
	 * @param totalCost
	 * @param totalReach
	 * @return json ErrorCode:<code>, ErrorResponce:<Responce> 0 means successfully updated -ve failed 
	 */
	public JsonEncoder modifyTargetsmsPStatus(String userId, String recordId, String status,String totalCost,String totalReach) {
		String errorCode="-1";	
	 ...
	 * @param totalCost
	 * @param totalReach
	 * @return Boolean True if both totalCost & totalReach is not null or empty
	 * else False
	 */
		...
		if(status.equalsIgnoreCase("2")) {
			...
			try {
				if(setNewTragetsmsPStatusCustomer(userId,recordId,status,oldstatus)) {
					...
					errorCode="3";
					errorRes="target SMS request update failed";
					LogWriter.LOGGER.info("Error encountered while updating Customer Status. bubblefyi.UserDBOperation.modifyTargetsmsPStatusCustomer");
				}				
	...
	public JsonEncoder BulksmsLowBalanceStatusChange( String customerId,String groupId) {
		...
		JsonEncoder jsonEncoder=new JsonEncoder();
		try {
			//check if group associated with user //also check the status //Check balance //Update payment							
	...
	public void transaction_logger(String userId,Double amount,int type,int action, String remark) {
		...
		String sql="INSERT INTO transaction_log"
				+ " (user_id,amount,type,action,remark) "
				+ "VALUES (?,?,?,?,?)";						
		try {
			bubbleDS.prepareStatement(sql,true);			
			bubbleDS.getPreparedStatement().setString(1,userId);
 ...
 */
			...
			if(isPriceAssociatedwithTrxID(trxID,price)){
				...
				if(purchase_status.equalsIgnoreCase("0")) {		
					...
					try {
						...
						updatePaymentInfoToCustomerBalance(userid,trxID,price,status);
						//TODO update payment log also update status
						//TODO email for successfull purchase
	...
	public boolean updatePaymentLog(String userID,String trxID,String price,String purchase_status,String oldFlag,String newFlag) {
		boolean retval=false;
		int output=-1;
		//LogWriter.LOGGER.info("userID:"+userID+" trxID:"+trxID+" price:"+price+" oldFlag:"+oldFlag+" newFlag:"+newFlag);
	...
	public JsonEncoder downloadSingleSMSReport(String userId, String start_date,String end_date,String output_type) {
		...
		String query="SELECT  t.bparty AS Receiver,t.aparty AS Sender,t.message AS SMS_Content,t.Sms_Count, t.source_id AS SID,\r\n" + 
				...
				"WHERE  t.userid = '"+userId+"'  AND t.insert_date between '"+start_date+"' and '" +end_date+"' \r\n" + 
				"ORDER BY t.ID DESC";
		try {
	...
	public boolean isUserAssociatedWithTrxID(String userid,String trxID) {
		...
		if(output.equalsIgnoreCase(userid))  retval=true;			
		return retval;
	...
	public boolean isPriceAssociatedwithTrxID(String trxID,String price) {
		...
		if(output.equalsIgnoreCase(price))  retval=true;			
		return retval;
 ...
 */
	...
	public String getBulkSMSDetailOfCustomer(String id){
		...
		/*
		String sql="SELECT group_id, CONVERT(message USING utf8) as message, sms_count,flag,DATE_FORMAT(scheduled_date, \"%d-%m-%Y %H:%i\") as scheduled_date,ifnull(DATE_FORMAT(done_date, \"%d-%m-%Y %H:%i\"),\"Processing\") done_date,  (select count(*) from groupsms_sender t where t.group_id=u.group_id) msisdnCount\r\n" + 
				" FROM groupsms_sender_info  u \r\n" + 
				" where user_id = ? ORDER BY group_id desc";/**/
